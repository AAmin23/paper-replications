...
modify_hr <- function(parm_VEs, parm_arm, parm_age, parm_infN, parm_infC) {
  ...
  cap <- (1-parm_VEs)^(parm_infC+1) # equal to vaccine+(0,1,2)inf
  
  if(parm_arm==0) {
    imm <- max(cap, (1-parm_VEs)^parm_infN)
  } else {
    imm <- max(cap, (1-parm_VEs)*((1-parm_VEs)^parm_infN))
  }
...
modify_rr_symptoms <- function(parm_VEp, parm_arm, parm_age, parm_infN, parm_infC) {
  ...
  cap <- (1-parm_VEp)^(parm_infC+1) # equal to vaccine+(0,1,2)inf
  
  if(parm_arm==0) {
    imm <- max(cap, (1-parm_VEp)^parm_infN)
  } else {
    imm <- max(cap, (1-parm_VEp)*((1-parm_VEp)^parm_infN))
  }
  update <- 1*imm*parm_age
  if(update > 0) {return(update)} else {return(0)}
}
...
modify_rr_severe <- function(parm_VEh, parm_arm, parm_age, parm_infN, parm_infC) {
  ...
  cap <- (1-parm_VEh)^(parm_infC+1) # equal to vaccine+(0,1,2)inf
  
  if(parm_arm==0) {
    imm <- max(cap, (1-parm_VEh)^parm_infN)
  } else {
    imm <- max(cap, (1-parm_VEh)*((1-parm_VEh)^parm_infN))
  }
  update <- 1*imm*parm_age
  if(update > 0) {return(update)} else {return(0)}
}